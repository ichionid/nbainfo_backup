...
that have been addressed there for a while. As some projects of mine need to
rely on a callback handler like this but collided with some implementations
in the original module, I decided to release this clone for at least until
the "original" callback has been revised.

That said, this project should be considered a workaround and will most likely
be abandoned once js.module has been revised.

The good thing, however: You don't need to alter anything. Just replace the
js folder in sites/all/modules with the js_callback folder and copy the
revised js.php into your drupal root directory as before. Also all integration
works as before, regarding hook_js() definitions and menu paths.

- Drupal 5 -

Note that there will be no support for D5 at the moment. Please refer to the
original module if you need that. 

- Drupal 6 -
 
Differences to the original module:
* Conditional re-implementation of the filter_xss_bad_protocol function
  (see http://drupal.org/node/1103394 ).
* Skipping some .htaccess directives that caused malfunctions with
  (deprecated) Apache 1.3 (see http://drupal.org/node/1103972 ).

- Drupal 7 - 

A development release is available which is basically a port of the Drupal 6 
version with various fixes based on the issues in the issuetracker of the js 
module. The current development is focused on the Drupal 7 version.


2. PERFORMANCE
==============

Note: The benchmarks are copied from the original js module.
...
Bug reports, feature suggestions and latest developments:
  http://drupal.org/project/issues/js_callback
...
The js_callback handler loads the least amount of code to function. This has the 
...
By default the js_callback module doesn't peform any kind of access check. 
...
Note on performance:
The js_callback module automatically bootstraps to DRUPAL_BOOTSTRAP_SESSION 
